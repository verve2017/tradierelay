...
export function CustomerPhotos({ accessToken = null, demo = false }: { accessToken?: string | null; demo?: boolean }) {
  ...
  async function addFiles(files: FileList | null) {
    ...
    for (const file of Array.from(files).slice(0, Math.max(0, 6 - photos.length))) {
      ...
      if (demo) {
        setPhotos((current) => [...current, { id: crypto.randomUUID(), name: file.name, preview, size: file.size }]);
        continue;
      }
  ...
  if (done) return <CustomerState title="Photos sent. Nice one." body="The tradie can now see them with the job details. You can close this page." success />;
...
function CustomerState({ title, body, success = false }: { title: string; body: string; success?: boolean }) {
  return <main className="customer-flow"><header><Link href="/" className="relay-app-brand">TradieRelay</Link></header><section className="customer-card customer-state"><span className={success ? 'customer-success-icon' : 'photo-icon'}>{success ? '✓' : '↗'}</span><h1>{title}</h1><p>{body}</p>{!success && <Link href="/book">Contact TradieRelay</Link>}</section></main>;
}
