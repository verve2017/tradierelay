  ...
  return <main className="relay-app-shell">
    ...
    <div className={`relay-workspace ${mobileDetailOpen ? 'is-detail-open' : ''}`}>
      ...
      {section === 'products' && canManageProducts ? <section className="relay-job-detail relay-products-detail" aria-live="polite"><button className="relay-mobile-back" type="button" onClick={() => setMobileDetailOpen(false)}>← Back to workspace</button><ProductManager products={products} onProductsChange={setProducts} demo={demo} onNotice={setNotice} /></section> : selected && <section className="relay-job-detail" aria-live="polite"><button className="relay-mobile-back" type="button" onClick={() => { setMobileDetailOpen(false); setScreen('detail'); }}>← Back to {sectionLabel(section).toLowerCase()}</button>
        ...
        {screen === 'edit_quote' && quote && <QuoteEditor quote={quote} products={products.filter((product) => product.active)} gstRegistered={workspace.gstRegistered} subtotal={subtotal} gst={gst} total={total} onChange={setQuote} onLineChange={updateLine} onAddLine={addLine} onAddProduct={addSavedProduct} onReview={() => void saveAndReview()} onBack={() => setScreen('detail')} error={error} />}
...
function JobDetail({ job, onCreateQuote }: { job: WorkspaceJob; onCreateQuote: () => void }) { return <><div className="relay-detail-head"><div><p>{job.status} · captured by AI</p><h2>{job.customer}</h2><span>{job.suburb}</span></div><a href={`tel:${job.phone.replaceAll(' ', '')}`} className="relay-call-link">Call {job.phone}</a></div>{['urgent', 'emergency'].includes(job.urgency) && <div className="relay-urgent-banner"><strong>{job.title}</strong><span>{job.urgency === 'emergency' ? 'Potential safety risk—review immediately' : 'Customer needs help promptly'}</span></div>}<div className="relay-ai-label"><span>✦</span> AI captured · check anything marked uncertain</div><div className="relay-detail-grid"><article><span>WHAT THEY NEED</span><p>{job.description}</p></article><article><span>CALL SUMMARY</span><p>{job.summary}</p></article>{job.photos.length > 0 && <article className="relay-customer-photos"><div className="relay-photo-heading"><div><span>CUSTOMER PHOTOS</span><strong>{job.photos.length} attached to this job</strong></div><small>Tap to open</small></div><div>{job.photos.map((photo, index) => <a key={photo.id} href={photo.url} target="_blank" rel="noreferrer"><Image unoptimized src={photo.url} alt={photo.caption || `Customer job photo ${index + 1}`} width={360} height={260} /><span>{photo.caption || `Photo ${index + 1}`}</span></a>)}</div></article>}<article><span>DETAILS CAPTURED</span><dl><div><dt>Job</dt><dd>{job.title}</dd></div><div><dt>Suburb</dt><dd>{job.suburb || 'Not confirmed'}</dd></div><div><dt>Urgency</dt><dd>{job.urgency}</dd></div><div><dt>Best time</dt><dd>{job.preferredWindows.join(' or ') || 'Not confirmed'}</dd></div></dl></article></div><div className="relay-detail-actions"><a href={`tel:${job.phone.replaceAll(' ', '')}`} className="relay-secondary-action">Call customer</a><button type="button" className="relay-primary-action" onClick={onCreateQuote}>{job.quote ? 'Open draft quote' : 'Create draft quote'}</button></div></>; }

function QuoteEditor({ quote, products, gstRegistered, subtotal, gst, total, onChange, onLineChange, onAddLine, onAddProduct, onReview, onBack, error }: { quote: QuoteState; products: SavedProduct[]; gstRegistered: boolean; subtotal: number; gst: number; total: number; onChange: (quote: QuoteState) => void; onLineChange: (index: number, changes: Partial<LineItem>) => void; onAddLine: () => void; onAddProduct: (id: string) => void; onReview: () => void; onBack: () => void; error: string | null }) { return <div className="relay-quote-editor"><div className="relay-editor-head"><button type="button" onClick={onBack}>← Job</button><div><span>DRAFT QUOTE</span><h2>Edit scope and price</h2></div></div><div className="relay-draft-warning">Draft only—the customer cannot see this yet.</div>{error && <p className="relay-form-error">{error}</p>}<div className="relay-saved-product-add"><label><span>ADD A SAVED PRODUCT</span><select value="" onChange={(event) => { if (event.target.value) onAddProduct(event.target.value); }}><option value="">Choose a saved item…</option>{products.map((product) => <option key={product.id} value={product.id}>{product.name} — {money(product.rateExGstCents)} / {product.unit}</option>)}</select></label><small>Its saved price is copied into this quote. You can still change it here.</small></div><div className="relay-line-items">{quote.lines.map((line, index) => <div className="relay-line-item" key={line.id ?? index}><label><span>Description</span><input value={line.description} onChange={(event) => onLineChange(index, { description: event.target.value })} /></label><label><span>Qty</span><input type="number" min="0.001" step="0.25" value={line.quantityMilli / 1000} onChange={(event) => onLineChange(index, { quantityMilli: Math.round(Number(event.target.value) * 1000) })} /></label><label><span>Rate ex GST</span><div className="relay-money-input"><b>$</b><input type="number" min="0" step="1" value={line.unitRateExGstCents === null ? '' : line.unitRateExGstCents / 100} onChange={(event) => onLineChange(index, { unitRateExGstCents: event.target.value === '' ? null : Math.round(Number(event.target.value) * 100) })} /></div></label></div>)}<button type="button" className="relay-add-line" onClick={onAddLine}>+ Add blank line item</button></div><label className="relay-note-field"><span>Note to customer (optional)</span><textarea maxLength={1000} value={quote.note} onChange={(event) => onChange({ ...quote, note: event.target.value })} placeholder="Add anything the customer should know…" /></label><div className="relay-totals"><div><span>Subtotal</span><b>{money(subtotal)}</b></div>{gstRegistered && <div><span>GST</span><b>{money(gst)}</b></div>}<div className="is-total"><span>Total {gstRegistered ? 'inc. GST' : ''}</span><b>{money(total)}</b></div></div><div className="relay-detail-actions"><button type="button" className="relay-primary-action" onClick={onReview}>Review quote</button></div></div>; }
